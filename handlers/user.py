...
from config import DB, TRIAL_DAYS, REF_BONUS, PAYMENT_REF_BONUS, MIN_WITHDRAW, SUPPORT, VPN_LINK, TRIAL_LINK, TARIFFS
...
async def start(message: types.Message):
    ...
    connect_kb = types.InlineKeyboardMarkup(inline_keyboard=[[
        types.InlineKeyboardButton(text="🔗 Подключить VPN (бесплатно)", url=TRIAL_LINK)
    ]])
    await message.answer(
        ...
        parse_mode="HTML", reply_markup=connect_kb)
...
async def show_profile(message: types.Message):
    ...
    if paid:
        ...
        kb = types.InlineKeyboardMarkup(inline_keyboard=[[
            types.InlineKeyboardButton(text="🔗 Подключиться (пробный)", url=TRIAL_LINK)
        ]])
    else:
        kb = None
    await message.answer(
